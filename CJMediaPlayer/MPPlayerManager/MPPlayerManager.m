...
@interface MPPlayerManager () <MPMediaPlayerDelegate>

@property (strong, nonatomic) MPMediaPlayer *mediaPlayer;
...
@property(weak, nonatomic) id <MPPlayerManagerDelegate> delegate;
...
+ (instancetype)sharePlayerManager {
    static MPPlayerManager *_sharePlayerManager = nil;
...
- (instancetype)init {
    if (self = [super init]) {
        _mediaPlayer = [MPMediaPlayer shared];
...
- (void)player:(MPMediaPlayer *)player statusDidChanged:(MPMediaPlayerStatus)status {
    if (MPMediaPlayerStatusReachEnd == status) {
        self.progress = 0.f;
        ++self.playCount;
    }
    if (MPMediaPlayerStatusReachEnd == status && self.loopPlay) {
        [self.mediaPlayer replay];
        if ([self.delegate respondsToSelector:@selector(playerManager:statusDidChanged:)]) {
            [self.delegate playerManager:self statusDidChanged:MPMediaPlayerStatusAutoReplay];
        }
    } else {
        if ([self.delegate respondsToSelector:@selector(playerManager:statusDidChanged:)]) {
