...
#import "MTAVResourceLoaderManager.h"
#import "MTAVUtil.h"
...
@interface MPMediaPlayer ()

@property(strong, nonatomic) AVPlayer *player;
@property(strong, nonatomic) AVPlayerItem *playerItem;
@property(strong, nonatomic) MTVideoView *videoView;
...
@property (nonatomic, strong) MTAVResourceLoaderManager *resourceLoaderManager;/** update by lichq */

@end

@implementation MPMediaPlayer {
    /**
     *  加载超时定时器
     */
...
- (void)commonInit {
    ...
    self.videoView = [[MTVideoView alloc] init];
    ...
    [end setDidEnterStateBlock:^(TKState *state, TKTransition *transition) {
        ...
        weakSelf.status = MPMediaPlayerStatusReachEnd;
        weakSelf.seeking = NO;
    ...
    [stop setDidEnterStateBlock:^(TKState *state, TKTransition *transition) {
        ...
        weakSelf.status = MPMediaPlayerStatusUnknow;
    ...
    [loading setDidEnterStateBlock:^(TKState *state, TKTransition *transition) {
        ...
        weakSelf.status = MPMediaPlayerStatusLoading;
    ...
    [playing setDidEnterStateBlock:^(TKState *state, TKTransition *transition) {
        [weakSelf.player play];
        weakSelf.status = MPMediaPlayerStatusPlaying;
    ...
    [pause setDidEnterStateBlock:^(TKState *state, TKTransition *transition) {
        ...
        weakSelf.status = MPMediaPlayerStatusPause;
...
- (void)loadMediaWithURL:(NSURL *)url {
    ...
    if (self.needCache) { /** 使用缓存时候 */
        self.resourceLoaderManager = [[MTAVResourceLoaderManager alloc] init];
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == &ItemLoadedTimeRangesContext) {
        ...
        if (self.isWillSeeking && self.status != MPMediaPlayerStatusReachEnd) {
            return;
        }
...
- (void)applicationDidBecomeActive:(NSNotification *)notification {
    if (![self readyForPlay]) {
        MPMediaPlayerStatus status = self.status;
        [self fireEvent:StopEventName userInfo:nil];
        if (status == MPMediaPlayerStatusLoading
                || status == MPMediaPlayerStatusPlaying) {
            [self fireEvent:LoadEventName userInfo:nil];
        }
    }
        ...
        if (self.status == MPMediaPlayerStatusPlaying) {
            [self.player play];
        }
    }
    _background = NO;
...
- (void)applicationWillResignActive:(NSNotification *)notification {
    if (self.status == MPMediaPlayerStatusPlaying
            || self.status == MPMediaPlayerStatusLoading) {
        _background = YES;
        [self.player pause];
    }
}
