...
@property (nonatomic, assign)NSInteger       bufferTimes;   //缓存次数



@property(nonatomic, strong) MTAVResourceLoaderManager *loaderManager;
...
//        NSString *cachePath = [MTAVResourceCache cachedPathByURL:url];
//        if ([MTAVResourceCache cacheFileExistsAtURL:url]) {
//            
//            self.videoAsset         = [AVURLAsset URLAssetWithURL:[NSURL fileURLWithPath:cachePath] options:nil];
...
//            
//            
//            self.loaderManager = [[MTAVResourceLoaderManager alloc] init];
...
//            
////            BOOL isIOS9 = [[[UIDevice currentDevice] systemVersion] compare:@"9.0"] != NSOrderedAscending;
////            if (isIOS9) {
////                self.curPlayerItem.canUseNetworkResourcesForLiveStreamingWhilePaused = YES;
////            }
//            
//            self.player = [AVPlayer playerWithPlayerItem:self.currentPlayerItem];
//        }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        if ([playerItem status] == AVPlayerStatusReadyToPlay) {
            ...
            NSLog(@"player__status:ReadyToPlay(已准备好，可以播放)");
            ...
            if ([playerItem status] == AVPlayerStatusFailed) {
                NSLog(@"player__status:AVPlayerStatusFailed(播放失败)");
            } else {
                NSLog(@"player__status:AVPlayerStatusUnknown(未知)");
            }
            [self stop];
        }
...
//        NSLog(@"player_status = %ld", playerItem.status);
//        if (playerItem.status == AVPlayerStatusReadyToPlay) {
//            [self monitoringPlayback:playerItem];// 给播放器添加计时器
//        }
        ...
        NSLog(@"player__status:playbackBufferEmpty(播放缓冲区空了)");
        
        
        
//        self.bufferTimes++;
//        if (self.bufferTimes % 2) {
//            [self seekToTime:self.playSlider.value + 1];
//            self.bufferTimes = 0;
//            return;
//        }
//        
//        if (!self.loaderManager.isLoading) {
//            [self seekToTime:self.playSlider.value + 1];
//            return;
//        }
...
- (void)bufferingSomeSecond
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        NSLog(@"buffer_____111111111111111");

        
        // 如果此时用户已经暂停了，则不再需要开启播放了
        if (self.isPauseByUser) {
            
            NSLog(@"buffer_____222222222222222");
        ...
        NSLog(@"buffer_____33333333333333");
        ...
        if (!self.currentPlayerItem.isPlaybackLikelyToKeepUp) {
            ...
            NSLog(@"buffer_____444444444444444444");
...
- (void)buildVideoNavBar
{
    ...
    if (!self.playSlider) {
        ...
        [_playSlider addTarget:self action:@selector(playSliderChangeStart:)
...
- (void)playSliderChangeStart:(UISlider *)slider
{
    [self.player pause];
}
