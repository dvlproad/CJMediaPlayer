...
#import "MTAVResourceLoaderTask.h"
...
@interface MTAVResourceLoaderTask ()
...
@implementation MTAVResourceLoaderTask
...
- (instancetype)initWithMediaUrl:(NSURL *)url {
    ...
    if (self) {
        ...
        NSString *cacheDescPath = [MTAVResourceCache cachedDescPathByURL:url];
        ...
        //创建并打开缓存文件
        [MTAVResourceCache createCacheFileAtPath:cachePath];
...
- (void)finishOrCancelRequest:(MTAVResourceRequest *)request {
    //从数组中删除，并计数减1
    [request cancel];
...
- (void)configResourceRequest:(MTAVResourceRequest *)request {
    ...
    weakRequest.resourceLoaderDataFinish = ^{
        ...
        if (weakSelf.loadingReqNum < weakSelf.requestArr.count) {
            MTAVResourceRequest *req = weakSelf.requestArr[weakSelf.loadingReqNum++];
            [req start];
        }
...
- (void)doLoadTaskWithDataBlockArr:(NSMutableArray *)blockArr
    ...
    if (!blockArr || blockArr.count == 0) {
        ...
        MTAVResourceRequest *request =
        [[MTAVResourceRequest alloc] initWithResourceInfo:self.curResourceInfo
                                                   Offset:0
                                                   Length:-1];
    ...
    MTAVResourceRequest *tempLoadingReq    = self.requestArr[0];
    MTAVResourceDataBlock *tempBlock       = blockArr[0];
    ...
    if (isBackward) {
        ...
        for (MTAVResourceRequest * req in beCancelRequestArr) {
            
            //如果该请求正在请求数据，则计数减1,
            if (req.reqStatus == MTAVResourceRequestStatus_Loading) {
                self.loadingReqNum --;
            }
        ...
        while (self.loadingReqNum < MaxConnectNum) {
            ...
            if (self.loadingReqNum < self.requestArr.count) {
                ...
                MTAVResourceRequest *tempReq = self.requestArr[self.loadingReqNum++];
                [tempReq start];
...
- (void)makeRequestFromBlockArr:(NSMutableArray *)blockArr {
    ...
    for (int i = 0; i < blockArr.count; ++i) {
        ...
        MTAVResourceDataBlock *dataBlock = blockArr[i];
        ...
        MTAVResourceRequest *curRequest =
        [[MTAVResourceRequest alloc] initWithResourceInfo:self.curResourceInfo
                                                   Offset:offset
                                                   Length:len];
        ...
        for (int j = 0; j < self.requestArr.count; ++j) {
            ...
            MTAVResourceRequest *loadRequest = self.requestArr[j];
...
- (void)cancel {
    ...
    for (MTAVResourceRequest *request in self.requestArr) {
        [request cancel];
    }
