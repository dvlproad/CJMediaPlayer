...
@interface MTAVResourceLoaderManager ()

@property(nonatomic, strong) NSMutableArray *pendingRequestArr;     //未决请求数组
@property(nonatomic, strong)MTAVResourceLoaderTask *loaderTask;     //当前loaderTask

@end


@implementation MTAVResourceLoaderManager
...
- (void)dealWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    NSURL *realURL = [MTAVUtil realUrl:fakeURL];

    //判断loaderTask是否存在，如果不存在，则创建新的实例
    if (!self.loaderTask) {

        __weak typeof(self) weakSelf = self;
        self.loaderTask = [[MTAVResourceLoaderTask alloc] initWithMediaUrl:realURL];
...
- (void)responseWholeDataForRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    NSString *path = [MTAVResourceCache cachedPathByURL:self.loaderTask.curResourceInfo.url];
