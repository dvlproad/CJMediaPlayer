...
typedef NS_ENUM(NSUInteger, CJAVResourceInfoStatus) {
    CJAVResourceInfoStatus_None,                //没有
    CJAVResourceInfoStatus_Partial,             //部分的
    CJAVResourceInfoStatus_Whole,               //完整的
};




@interface CJAVResourceInfo : NSObject
...
/** 下载状态 */
@property(nonatomic, assign) CJAVResourceInfoStatus status;
...
/**
 *  从缓存描述文件中实例化CJAVResourceInfo
 *
 *  @param descPath 缓存描述文件的路径
 *
 *  @return CJAVResourceInfo实例对象
 */
+ (CJAVResourceInfo *)resourceInfoFromCacheWithDescPath:(NSString *)descPath;
