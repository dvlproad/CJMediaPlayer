...
@interface CJAVResourceRequest ()<NSURLSessionDataDelegate>
...
- (instancetype)initWithResourceInfo:(MTAVResourceInfo *)resourceInfo
    ...
    if (self) {
        ...
        self.reqStatus          = CJAVResourceRequestStatus_initd;   //初始化状态
...
- (void)start {
    self.reqStatus = CJAVResourceRequestStatus_Loading;    //下载状态
...
- (void)cancel {
    ...
    self.reqStatus = CJAVResourceRequestStatus_Cancel;      //取消状态
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    ...
    if (httpResponse.statusCode >= 400) { //[contentType isEqualToString:@"text/html"]
        ...
        self.reqStatus = CJAVResourceRequestStatus_Finish;      //结束状态
...
- (void)dataLoadDidFinish {
    ...
    self.reqStatus = CJAVResourceRequestStatus_Finish;      //请求结束
